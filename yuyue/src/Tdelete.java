...
public class Tdelete extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		String[] tslt=request.getParameterValues("T_dlt");
		...
		if(T_c != null)
		{
			...
			for (int i = 0; i< tslt.length; i++)
			{
				...
				if(flg == true)
					{
					ttmp += String.valueOf( di) + String.valueOf( j);
			...
			for( int i = 1; i < fabu.length(); i += 2)
			{
				flg = true;
				for( j = 1; j < ttmp.length(); j += 2)
				{
					if( (fabu.charAt(i - 1) == ttmp.charAt(j - 1)) && (fabu.charAt(i) == ttmp.charAt(j)))
					{
						flg = false;
						break;
					}
